...
class CustomAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        attn: Attention,
        hidden_states: torch.Tensor,
        encoder_hidden_states: Optional[torch.Tensor] = None, #num of texts * batch size * ...
        attention_mask: Optional[torch.Tensor] = None,
        temb: Optional[torch.Tensor] = None,
        ...
        if len(hidden_states.shape) == 3:
            ...
            for index in range(hidden_states.shape[1]):
                ...
                batch_size, sequence_length, _ = (
                    _hidden_states.shape if encoder_hidden_states is None else encoder_hidden_states.shape
                )
...
class IPAdapterAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        print(f'ip_hidden_states len = {len(ip_hidden_states)}')
        for current_ip_hidden_states, scale, to_k_ip, to_v_ip, mask in zip(
            ip_hidden_states, self.scale, self.to_k_ip, self.to_v_ip, ip_adapter_masks
        ):
            skip = False
