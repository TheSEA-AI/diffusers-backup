    ...
    def __call__(
            ...
            if img_mask is not None:
                ...
                if not prod_masks.shape[0] == int(encoder_hidden_states.shape[1]/512):
                    raise ValueError(
                        ...
                        f"the number of text prompts "
                        f"({int(encoder_hidden_states.shape[1]/512)})")
                queries = []
                keys = []
                values = []
        ...
        if encoder_hidden_states is not None: 
            ...
            if img_mask is not None:
                ...
                masked_img_hidden_states = img_hidden_states[:,512:,:] * img_mask_downsample
                ...
                hidden_states[:,:-4608,:] = encoder_hidden_states
            else:
                hidden_states = F.scaled_dot_product_attention(
                    query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
                )
            ...
            if ((hidden_states.shape[1] - 4096) > 512) and (prod_masks is not None):
                if (hidden_states.shape[1] - 4096) % 512 == 0:
                    ...
                    encoder_hidden_states_list = torch.cat(encoder_hidden_states_list, dim=1)
                    hidden_states = torch.cat([encoder_hidden_states_list, hidden_states],dim=1)
                    hidden_states[:,-4608:-4096,:] = encoder_hidden_states
                else:
                    hidden_states = F.scaled_dot_product_attention(
                        query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
                    )
