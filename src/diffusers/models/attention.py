    ...
    def forward(
        ...
        if len(hidden_states.shape) == 3:
            ...
            for index in range(hidden_states.shape[1]):
                if self.norm_type == "ada_norm":
                    norm_hidden_states = self.norm1(hidden_states[:,index,:,:], timestep)
                elif self.norm_type == "ada_norm_zero":
                    norm_hidden_states, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.norm1(
                        hidden_states[:,index,:,:], timestep, class_labels, hidden_dtype=hidden_states.dtype
                    )
                elif self.norm_type in ["layer_norm", "layer_norm_i2vgen"]:
                    norm_hidden_states = self.norm1(hidden_states[:,index,:,:])
                elif self.norm_type == "ada_norm_continuous":
                    norm_hidden_states = self.norm1(hidden_states[:,index,:,:], added_cond_kwargs["pooled_text_emb"])
                elif self.norm_type == "ada_norm_single":
                    shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
                        self.scale_shift_table[None] + timestep.reshape(batch_size, 6, -1)
                    ).chunk(6, dim=1)
