...
from ..attention import BasicTransformerBlock, CustomBasicTransformerBlock
...
class Transformer2DModel(LegacyModelMixin, LegacyConfigMixin):
    ...
    def forward(
        ...
        len_of_hidden_states = len(hidden_states.shape)
        # 1. Input
        if self.is_input_continuous:
            batch_size, _, height, width = hidden_states.shape if len_of_hidden_states == 4 else hidden_states[:,0,:,:,:].shape
            if len_of_hidden_states == 4:
                residual = hidden_states
                hidden_states, inner_dim = self._operate_on_continuous_inputs(hidden_states)
            else:
                hidden_states_list = []
                ...
                for index in range(hidden_states.shape[1]):
                    _hidden_states, inner_dim = self._operate_on_continuous_inputs(hidden_states[:,index,:,:,:])
